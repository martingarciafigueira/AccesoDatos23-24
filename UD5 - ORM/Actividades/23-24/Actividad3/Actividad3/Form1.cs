...
namespace Actividad3
{
    public partial class Form1 : Form
    {
        ...
        public void InsertarEquipo(Equipo equipo)
        {
            using (IDbConnection conexion = new SqlConnection(cadenaConexion))
            {
                var consulta = $@"INSERT INTO Equipos (Codigo, Nombre, Pais, Estadio, Ciudad) VALUES (@Codigo, @Nombre, @Pais, @Estadio, @Ciudad)";

                //Ejecucion de la consulta
                conexion.Execute(consulta, equipo);
            }
        }
        public void InsertarFutbolista(Futbolista futbolista)
        {
            using (IDbConnection conexion = new SqlConnection(cadenaConexion))
            {
                var consulta = $@"INSERT INTO Futbolistas (Codigo, Nombre, CodigoEquipo, Posicion, Edad, Dorsal) VALUES (@Codigo, @Nombre, @CodigoEquipo, @Posicion, @Edad, @Dorsal)";

                //Ejecucion de la consulta
                conexion.Execute(consulta, futbolista);
            }
        }
        public void EditarEquipo(Equipo equipo)
        {
            ...
            {
                var consulta = 
                    $@"UPDATE Equipos SET Codigo = @Codigo, 
        ...
        public void EditarFutbolista(Futbolista futbolista)
        {
            ...
            {
                var consulta =
                    $@"UPDATE Futbolistas SET Codigo = @Codigo, 
        ...
        public void ActualizarListaFutbolistas()
        {
            using (IDbConnection conexion = new SqlConnection(cadenaConexion))
            {
                ...
                var consulta = $@"SELECT * FROM Futbolistas WHERE CodigoEquipo = '{equipo.Codigo}'";
